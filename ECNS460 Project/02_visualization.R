...
stations = read_csv("Raw Data/alt_fuel_stations.csv")
...
fig1 = ggplot(fig1_data, aes(x = evs_on_road, y = county_label, fill = state)) +
  ...
  geom_text(aes(label = paste0(round(evs_on_road / 1000))), hjust = -0.1, size = 3) +
  ...
  labs(
    ...
    x = "EVs on the Road (in thousands)",
    y = NULL
  ) +
  theme_ev +
...
save(visualizations, file = "clean_data/visualizations.RData")
